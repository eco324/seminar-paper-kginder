...
date_var <= as.Date("2020-04-05") ~ 2,
date_var <= as.Date("2020-05-03") ~ 3))
View(Compost_data)

export_summary_table(dfSummary(Compost_data))
```
...
```{r}
ggplot(data = Compost_data, aes(x = house, y = compost)) + geom_point() + geom_smooth(method = "lm")
```

```{r}
ggplot(data = Compost_data, aes(x = cycle_number, y = (cutter_compost/cutter_plates))) + geom_point() + geom_smooth(method = "lm")
```
...
There was not any surprising data recorded in the summary statistics.  Both dinning halls had similar plate counts for minimum and maximum values.  This was also true for the amount of compost tracked as well.  They had almost exactly the same mean 42.16 verse 42.79.   
